...
Resource::Resource()
    : nodeEnvironment(nullptr, node::FreeEnvironment), asyncContext{} {}
...
void Resource::Init() {
  ...
  auto env = node::CreateEnvironment(sampnode::nodeImpl.GetNodeIsolate(),
                                     context, {}, // args
                                     {},          // exec_args
                                     flags);
  ...
  if (env) {
    ...
    asyncContext =
        node::EmitAsyncInit(isolate, asyncResourceObj, "sampNodeResource");
  } else {
