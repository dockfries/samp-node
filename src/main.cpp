...
PLUGIN_EXPORT bool PLUGIN_CALL Load(void **ppData) {
  ...
  if (!mainConfig.ParseFile("samp-node")) {
    L_ERROR << "Unable to load samp-node config file, you need to have "
               "samp-node.json in root directory"
            << std::endl
            << "\tPlease read about samp-node api at "
               "https://github.com/dockfries/samp-node";
    return false;
  }
